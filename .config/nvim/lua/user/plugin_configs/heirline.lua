...
local RightSeparator = { provider = '  ', hl = { fg = 'light_gray_bright' } }
...
local function setup_colors()
  return {
    ...
    dark_bg = utils.get_highlight("StatusLineNC").bg,
    ...
    dark_blue = utils.get_highlight("Constant").fg,
...
local ViMode = {
  ...
  --   return { fg = self.mode_fg_colors[mode], bold = true, }
  -- end,
  {
    -- {
    --   provider = "",
    ...
    -- },
    {
      ...
      hl = function(self)
        local mode = self.mode:sub(1, 1) -- get only the first mode character
        return { fg = self.mode_fg_colors[mode], bg = self.mode_bg_colors[mode], bold = true, }
      end,
    },
    {
      -- provider = "",
...
FileIcon = utils.insert(FileNameContext, { FileIcon })
...
local WinBarFileName = {
  init = function(self)
    ...
    for path in string.gmatch(filename, "[^/]+") do
      table.insert(splitedData, path)
    end
    for i, path in ipairs(splitedData) do
      local child = {}
      if i == #splitedData then
        table.insert(child, FileIcon)
      end
      table.insert(child, {
        provider = path,
        hl = function()
      ...
      if #splitedData > 1 and i ~= #splitedData then
        table.insert(child, {
          provider = ' > ',
        })
      end
...
local WinBarFileNameBlock = {
  utils.insert(FileNameContext, WinBarFileName)
}
...
local Navic = {
  condition = function() return conditions.is_active() and require("nvim-navic").is_available() end,
  static = {
...
local DefaultStatusline = {
  ViMode, Space, FileNameBlock, Space, Git, Space, Diagnostics, Space, LSPActive, Align,
  DAPMessages, Align,
  FileLastModified, RightSeparator, ProjectSpace, RightSeparator, FileSize, RightSeparator, FileEncoding, Space, FileIcon, FileType, Space, FileFormat, RightSeparator, ScrollBar, Space, Ruler
}
...
local SpecialStatusline = {
  condition = function()
    return conditions.buffer_matches({
      ...
      filetype = { "^git.*", "fugitive", 'alpha', 'dashboard', 'NvimTree', 'Outline' },
    })
  end,
...
local WinBars = {
  ...
  {   -- An inactive winbar for regular files
    ...
    utils.surround({ "", "" }, "dark_gray", { hl = { fg = "light_gray_bright", force = true }, WinBarFileNameBlock }),
  },
  -- A winbar for regular files
  utils.surround({ "", "" }, "bright_bg", { WinBarFileNameBlock, Navic }),
}
...
require('heirline').setup({
  ...
  opts = {
    ...
    disable_winbar_cb = function(args)
      return vim.fn.fnamemodify(vim.api.nvim_buf_get_name(0), ':.') == '' or conditions.buffer_matches({
        buftype = { "nofile", "prompt", "help", "quickfix" },
        filetype = { "^git.*", "fugitive", "Trouble", "dashboard", "neo-tree", "NvimTree" },
      }, args.buf)
    end,
    colors = colors,
  },
